"""End-to-end RAG eval on NFCorpus: baseline vs HyDE-best (N=8 qry q+).
...
OUT = Path("results/e2e_nfcorpus"); OUT.mkdir(parents=True, exist_ok=True)
...
hyde_docs = json.load(open("results/hyde/hyde_docs.json"))
hyde_vec = {}
for q in sample:
    docs = hyde_docs[q][:8]
    hd = emb(docs, "query").mean(axis=0)
    v = (hd + qmat[q]) / 2.0; hyde_vec[q] = v / (np.linalg.norm(v) + 1e-12)
...
tbl = "\n".join(lines)
doc = ("# NFCorpus — End-to-End RAG Answer Quality (reference-free)\n\n**Reference setup**\n\n| Field | Value |\n|---|---|\n"
       f"| Dataset | NFCorpus (BEIR) — {len(sample)}-query sample |\n"
